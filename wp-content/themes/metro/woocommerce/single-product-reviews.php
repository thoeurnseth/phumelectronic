...
<div id="rdtheme-wc-reviews" class="rdtheme-wc-reviews">
	<div id="comments">
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php esc_html_e( 'There are no reviews yet.', 'metro' ); ?></p>
	...
	<?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->get_id() ) ) : ?>
		...
		<?php
			...
			$comment_form = array(
				'title_reply'          => have_comments() ? esc_html__( 'Add a review', 'metro' ) : sprintf( esc_html__( 'Be the first to review &ldquo;%s&rdquo;', 'metro' ), get_the_title() ),
				'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'metro' ),
				'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title">',
				'title_reply_after'    => '</h3>',
				'comment_notes_after'  => '',
				'fields'               => array(
					'author' => '<p class="comment-form-author">' . '<label for="author">' . esc_html__( 'Name', 'metro' ) . '&nbsp;<span class="required">*</span></label> ' .
				...
				'label_submit'  => esc_html__( 'Submit', 'metro' ),
				'logged_in_as'  => '',
				'comment_field' => '',
			);
			...
			if ( wc_review_ratings_enabled() ) {
				$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'metro' ) . '</label><select name="rating" id="rating" required>
