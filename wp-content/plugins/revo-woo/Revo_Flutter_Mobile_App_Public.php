...
class Revo_Flutter_Mobile_App_Public
{
    ...
    function reformat_product_result($product)
    {
		...
		foreach ( $product->get_children( 'view' ) as $child_id ) {
        ...
        if($_product) {
            ...
            $date_on_sale_from  = wc_rest_prepare_date_response( $_product->get_date_on_sale_from( $context ), false );
            $date_on_sale_to    = wc_rest_prepare_date_response( $_product->get_date_on_sale_to( $context ), false );

            if(!empty($date_on_sale_from) && !empty($date_on_sale_to)){
                $now        = date("YmdHis");
                $start_date = date("YmdHis", strtotime($date_on_sale_from));
                $end_date   = date("YmdHis", strtotime($date_on_sale_to));
                if($start_date <= $now && $now <= $end_date){
                    $sale_price = $_product->get_sale_price($context);
                }
                else {
                    $sale_price = '';
                }
        ...
        if($sale_price == '' || $sale_price == 0) {
            ...
            $get_price = $sale_price;
        }

        return array(
            'id' => $product->get_id(),
            'name' => $product->get_name(),
            'sku' =>  $_product->get_sku( 'view' ),//'Songhuy',//$product->get_sku( 'view' ),
            ...
            'price' => (double)$get_price,//$product->get_price(),
            'regular_price' => (double)$_product->get_regular_price(),//(double)$product->get_regular_price(),
            'sale_price' => (double)$sale_price,//(double)$_product->get_sale_price(),//(double)$product->get_sale_price(),
            ...
            'variations'            => array($available_variations[0]['variation_id']),
    ...
    public function get_formatted_item_data( $object ) {
        ...
        $order_sub_total = array_column($data['line_items'], 'subtotal')[0];
        $order_discount = array_column($data['coupon_lines'], 'discount')[0] != '' ? array_column($data['coupon_lines'], 'discount')[0] : "0";
		...
		return array(
            ...
            'status'               => $data['status'],
            'sub_total'            => html_entity_decode($object->get_subtotal()),
