...
class WC_REST_Products_V2_Controller extends WC_REST_CRUD_Controller {
	...
	protected function get_product_data( $product, $context = 'view' ) {
		...
		*/
		...
		/*****************************************************
		 ******* Biz custome for get sku in variations *******
		 *****************************************************/
		if($context === 'view'){
			$object = $this->get_object( (int) $product->get_id() );
		...
		foreach ( $fields as $field ) {
			switch ( $field ) {
				...
				case 'short_description':
					...
					break;
				case 'sku':
					$base_data['sku'] = 'view' === $context ? $_product ? $_product->get_sku( $context ): $product->get_sku( $context ) : $product->get_sku( $context );//Biz
					break;
				case 'price':
					if($_product){
						$base_data['price'] = $_product->get_price( $context );
					}else{
						$base_data['price'] = $product->get_price( $context );
					}
					break;
				case 'regular_price':
					$base_data['regular_price'] = 'view' === $context? $_product ? $_product->get_regular_price( $context ) : $product->get_regular_price( $context ) : $product->get_regular_price( $context ); //$product->get_regular_price( $context );
					break;
				case 'sale_price':
					...
					if($_product){
						...
						if(!empty($date_on_sale_from) && !empty($date_on_sale_to)){
							...
							$start_date = date("Y-m-d H:i:s", strtotime($date_on_sale_from));
							$end_date   = date("Y-m-d H:i:s", strtotime($date_on_sale_to));
							if($start_date <= $now && $now <= $end_date){
								$base_data['sale_price'] = $_product->get_sale_price($context);
							}
						}elseif(!empty($date_on_sale_from)){
							$now        = date("Y-m-d H:i:s");
							$start_date = date("Y-m-d H:i:s", strtotime($date_on_sale_from));
							if($start_date <= $now){
								$base_data['sale_price'] = $_product->get_sale_price($context);
