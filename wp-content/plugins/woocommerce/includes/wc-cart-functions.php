...
function generate_cart_id( $product_id, $variation_id = 0, $variation = array(), $cart_item_data = array()) {
...
function wc_get_cart_by_user($user_id){
	
	if ( ! $user_id || ! apply_filters( 'woocommerce_persistent_cart_enabled', true ) ) {
		return;
	}
	...
	if ( empty( $cart ) || ! is_array( $cart ) || 0 === count( $cart ) ) {
		$cart =	WC()->session->cart = $saved_cart['cart'];
	}
	$cart_items = array_filter($cart);
...
function add_multi_cart_by_user($user_id,$products){
	$add = false;
	if($products){
		for($i = 0; $i < count($products); $i++){
		 $add =	wc_add_to_cart_by_user($user_id,$products[$i]->product_id,(int)$products[$i]->quantity,$products[$i]->variation_id);
		}
	}
	return $add;
}
...
function wc_update_cart_by_user($user_id, $item_id,$qty){
	$udapte = false;
	if(is_array($item_id) == true){
		for($i = 0; $i < count($item_id); $i++){
			$saved_cart = get_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), true );
			unset($saved_cart['cart'][$item_id[$i]]);
			$udapte = update_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), $saved_cart );
		}	
	}else{
		if($qty > 0){
			$saved_cart = get_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), true );
			$saved_cart['cart'][$item_id]['quantity'] = (int)$qty;
			$udapte = update_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), $saved_cart );	
		}else{
			$saved_cart = get_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), true );
			unset($saved_cart['cart'][$item_id]);
			$udapte = update_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), $saved_cart );	
		}
	}
	return $udapte;
...
function wc_clear_cart_by_user($user_id){
	$delete = update_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), ['cart'=>[]] );	
	return $delete;
}

function wc_add_to_cart_by_user($user_id,$prod_id,$qty,$variation_id) {
	...
	//$response = json_decode(rest_ensure_response( $items ));

	$update = false;
	for($i = 0; $i < count($items); $i++){
		if((int)$items[$i]['product_id'] == (int)$prod_id){
			$qty = $items[$i]['quantity']  + $qty;
			$update = wc_update_cart_by_user($user_id, $items[$i]['key'],$qty);
		}
	}

	if($update == false){
		$cart_id = generate_cart_id( $prod_id,$variation_id );
		...
		$saved_cart['cart'] =  array_merge(
			$saved_cart['cart']? $saved_cart['cart'] : array(),
			array( $cart_id => $new_cart)
		);
	
		 $update = update_user_meta( $user_id, '_woocommerce_persistent_cart_' . get_current_blog_id(), $saved_cart );
	}
