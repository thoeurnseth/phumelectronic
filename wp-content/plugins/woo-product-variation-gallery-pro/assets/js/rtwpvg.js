(function ($) {
  ...
  $.fn.rtWpVGallery = function () {
    ...
    this.loadPhotoSwipe = function () {
      ...
      this._item.off('click', '.rtwpvg-trigger'); // this._item.off('click', '.rtwpvg-gallery-image a');
    ...
    this.preloadVariationImages = function () {
      if (this._is_variation_product) {
        var that = this;
        wp.ajax.send('rtwpvg_get_variation_images', {
          data: {
            product_id: this._product_id
          },
          success: function success(images) {
            that._variation_images = images;
          },
          error: function error(e) {
            that._variation_images = [];
          }
        });
      }
    };
