...
use Rtwpvg\Controllers\Licensing;
...
if (!class_exists('WooProductVariationGalleryPro')):
    final class WooProductVariationGalleryPro
    {
        ...
        public function define_constants() {
            $this->define('RTWPVG_PRO_PLUGIN_PATH', plugin_dir_path(RTWPVG_PRO_PLUGIN_FILE));
            $this->define('RTWPVG_PRO_PLUGIN_URI', plugin_dir_url(RTWPVG_PRO_PLUGIN_FILE));
            $this->define('RTWPVG_PRO_PLUGIN_DIRNAME', dirname(plugin_basename(RTWPVG_PRO_PLUGIN_FILE))); // plugin-slug
            $this->define('RTWPVG_PRO_PLUGIN_BASENAME', plugin_basename(RTWPVG_PRO_PLUGIN_FILE)); // plugin-slug/plugin-slug.php
        }
        ...
        public function basename() {
            return RTWPVG_PRO_PLUGIN_BASENAME;
        }


        public function dirname() {
            return RTWPVG_PRO_PLUGIN_DIRNAME;
        }

        public function version() {
            return RTWPVG_PRO_VERSION;
        }

        public function author() {
            return RTWPVG_PRO_AUTHOR;
        }
        ...
        public function get_assets_uri($file) {
            ...
            return trailingslashit(RTWPVG_PRO_PLUGIN_URI . 'assets') . $file;
        }

        public function get_images_uri($file) {
            $file = ltrim($file, '/');

            return trailingslashit(RTWPVG_PRO_PLUGIN_URI . 'assets/images') . $file;
        }

        public function get_template_file_path($file) {
            $file = ltrim($file, '/');

            return trailingslashit(RTWPVG_PRO_PLUGIN_PATH . 'templates') . $file . ".php";
        }
    ...
    /**
     * @return null|WooProductVariationGalleryPro
     */
    function rtwpvg() {
        return WooProductVariationGalleryPro::get_instance();
    }

    register_activation_hook(RTWPVG_PRO_PLUGIN_FILE, array(Install::class, 'activated'));
    register_deactivation_hook(RTWPVG_PRO_PLUGIN_FILE, array(Install::class, 'deactivated'));
