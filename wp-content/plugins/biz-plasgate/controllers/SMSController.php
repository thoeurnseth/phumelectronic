...
class SMSController
{
    ...
    public function send_otp()
    {
        ...
        try{
            ...
            echo "error send sms:". $e->getMessage();
        }
    ...
    {       
        ...
        if( count($phones) > 0 ):
            ...
            $user_id = '';
            $user_metas = $phone->wpdb->get_results("SELECT * FROM ph0m31e_usermeta WHERE meta_key = 'phone_number' AND meta_value = '".$_POST['phone']."'");
            if(count($user_metas)){
                $user_id = $user_metas[0]->user_id;
            }

            if(!empty($user_id)){
                $user_meta = $phone->wpdb->get_results("SELECT * FROM ph0m31e_usermeta WHERE user_id=".$user_id." AND meta_key='register_status'");
                if(count($user_meta)){
                    $phone->wpdb->update('ph0m31e_usermeta', array('meta_value'=>1), array('user_id'=>$user_id , 'meta_key' => 'register_status'));
                }else{
                    $phone->wpdb->insert('ph0m31e_usermeta', array('user_id'=>$user_id , 'meta_value'=>1, 'meta_key' => 'register_status'));
                }
            }
