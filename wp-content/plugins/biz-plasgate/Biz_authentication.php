...
//		$params = wp_unslash( $params );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $params ) ) {
			$this->set_error( new WP_Error( 'revo_rest_authentication_error', __( '1. Consumer key is invalid.', 'woocommerce' ), array( 'status' => 401 ) ) );
			return false;
		}
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if( $params["email"]){
			$params["email"] = str_replace("%40","@",$params["email"]);
		}
